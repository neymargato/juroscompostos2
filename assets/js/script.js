// Esta linha seleciona o elemento HTML com ID 'calcule' e adiciona um ouvinte de evento de clique (click).
document.getElementById('calculate').addEventListener('click', function(){
    ...
    //O resultado é armazenado na variável 'feee.
    const fee = (document.getElementById('fee').value) / 100;

    // Esta linha obtém o valor digitado no elemento HTML com ID 'time' e armazena na variável 'time'.
    const time = document.getElementById('time').value;

    // Esta linha calculada o total com base nos valores obtidos anteriormente. O cálculo é realizado usando a 
    //fórmula de juros compostos.
    const total = value * (1 + fee)**time;

    // Esta linha ultiliza o conteúdo do elemento HTML com o ID 'total' para mostrar o resultado formatado
